...
| **N6d-7** | ⑥.4 sort+dedup | **Tại sao:** Sau 3 lớp filter (negation + price + category), SP còn lại cần sắp xếp theo relevance. Sort matchScore desc → SP match nhiều keyword nhất lên đầu. Dedup loại SP trùng ID — input từ hybridSearch (bước ⑤b) đã merge vector + keyword results, nhưng 1 SP có thể score từ nhiều token match → tạo duplicate trong quá trình scoring | `keyword-fallback.js` |
