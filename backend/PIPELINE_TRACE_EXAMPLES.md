...
| **G1** | prompt injection? | **Tại sao:** Gate 1 check TRƯỚC Gate 2 (off_topic) vì injection nguy hiểm hơn — cần chặn ngay dù intent có vẻ "bình thường". Injection message thường có intent = `general` (không match pattern sản phẩm) | `chatbot-service.js:247` |
| **G2** | offTopic? | **Tại sao:** Tiết kiệm chi phí LLM (~$0.003/request) + retrieval cho query chắc chắn ngoài phạm vi (thời tiết, bóng đá...). Dùng regex < 1ms thay vì gọi LLM 1-3s để phân loại | `chatbot-service.js:264` |
...
| **N5a** | ⑤a enrichQuery | **Tại sao:** Vector search không hiểu đại từ — search "cái đó có bao nhiêu RAM" trả kết quả ngẫu nhiên. Trigger khi: (1) query chứa đại từ (cái đó/này/kia, nó, so sánh, cả hai), hoặc (2) implicit follow-up (query ≤50 chars + không có brand). Extract tên SP đầu tiên từ 1-2 assistant messages gần nhất → append vào query → embedding gần đúng SP | `chatbot-service.js:362` |
| **N5b-1** | ⑤b strip negation | **Tại sao:** Embedding không hiểu negation — vector("không muốn iPhone") **gần** vector("iPhone") vì từ "iPhone" vẫn xuất hiện. Strip mệnh đề phủ định ("không cần/muốn/thích/dùng X", "tránh X", "avoid X") khỏi `queryForRetrieval` trước search → tránh embedding bias. Lưu ý: strip rộng hơn N6d-4 filter (bao gồm cả "không cần" — vì dù không phải loại trừ, vẫn gây bias embedding) | `chatbot-service.js` |
| **N5b-2** | ⑤b Promise.all | **Tại sao:** LLM rewrite mất 3-8s, hybridSearch mất ~0.5s. Nếu chạy tuần tự (rewrite xong → mới search) tổng 3.5s. Chạy **song song** → tổng chỉ 3s. Search lần 1 dùng `queryForRetrieval` (query đã strip negation ở N5b-1). Kết quả search lần 1 được dùng trong 3 trường hợp: (1) rewrite fail/timeout, (2) rewrite giống normalizedQuery → skip search lần 2, (3) search lần 2 rỗng → fallback. Nên search lần 1 không bao giờ lãng phí | `chatbot-service.js:434` |
...
| **N5b-4** | products > 0? | **Tại sao:** Nếu 0 SP có cosine similarity ≥ 0.45 (ngưỡng mặc định `DEFAULT_MIN_SCORE` — score dưới 0.45 nghĩa là SP không liên quan), LLM không có context → không trả lời được. Hạ ngưỡng xuống 0 → lấy top 3 SP dù score thấp + đánh flag `lowConfidence=true` → prompt builder thêm `⚠️[low confidence]` để LLM biết "đây không chắc chắn" → LLM có thể trả "chưa có SP này" thay vì hallucinate | `chatbot-service.js` |
| **N6-check** | ⑥ providers? | **Tại sao:** Điểm rẽ chính quyết định chất lượng response. `providers.length===0` (không có LLM API nào) → LLM DOWN (keyword fallback, chất lượng thấp hơn nhưng luôn hoạt động). Ngược lại → LLM UP (RAG đầy đủ, natural language response) | `chatbot-service.js:609` |
...
| **N6b-1** | ⑥ LLM HTTP POST | **Tại sao:** Bước **Generate** (chữ G trong RAG). temp=0.3 cho deterministic (tránh creative hallucination). max_tokens=800 đủ cho JSON response chi tiết. Provider rotation: 429/402/500/503/network error (lỗi tạm thời) → `continue` thử provider tiếp. 400/401 (lỗi cố định — bad request/unauthorized) → `break` ngay, không retry | `chatbot-service.js` |
| **N6b-2** | ⑥ parseLLMOutput | **Tại sao:** LLM có thể hallucinate tên SP không tồn tại trong DB. Parse JSON → match tên SP trong `matchedProducts` với danh sách products thực từ bước ⑤ → loại SP hallucinate. Ngoài ra, LLM có thể nhắc tên SP trong phần `response` text (ví dụ: "Bạn nên xem iPhone 17 Pro") nhưng không liệt kê trong `matchedProducts` array → `extractProductsFromText` quét response text, tìm tên SP khớp, bổ sung vào kết quả | `response-parser.js` |
...
| **N6d-1** | ⑥.1 tokenize+score | **Tại sao:** Khi không có LLM, dùng keyword matching thay thế. `product.name` match +10 > `product.shortDescription` match +5 vì tên SP quan trọng hơn mô tả ngắn: "iPhone 17" trong tên = chắc chắn đúng SP, trong mô tả = có thể là SP khác nhắc tới | `keyword-fallback.js:66` |
...
| **N6d-3** | ⑥.2 brand coherence | **Tại sao:** User hỏi "Huawei Mate 70" → search trả Samsung/iPhone (vì embedding gần "điện thoại"). Nếu không check → recommend Samsung cho người hỏi Huawei = sai. Cách hoạt động: lấy token đầu tiên trong query (dài >3 chars, không phải số, có trong SP ban đầu) làm `brandDiscriminator` → check xem token đó có trong kết quả **sau version filter** không → nếu không → `notFoundResponse` | `keyword-fallback.js:175-183` |
...
| **N6d-4** | ⑥.3 negation filter | **Tại sao:** Ở bước N5b-1, negation bị strip khỏi `queryForRetrieval` (chỉ dùng cho embedding search). Nhưng `finalQuery` truyền vào `simpleKeywordMatch` là **enrichedQuery hoặc llmRewrite** — cả hai đều giữ nguyên negation. Nên N6d-4 vẫn đọc được "không muốn/thích/dùng", "tránh", "avoid" → loại SP bị phủ định. Lưu ý: "không **cần**" KHÔNG nằm trong negation filter (code coi "không cần X" = brand không quan trọng, không phải loại trừ). Chỉ nhận pattern cứng — không hiểu ẩn ý ("chán Samsung" không bị bắt) | `keyword-fallback.js` |
...
| **N6d-6** | ⑥.3 category prefix | **Tại sao:** User hỏi "laptop 20 triệu" → search trả cả điện thoại 20 triệu. Category prefix filter: detect term "laptop" trong query → check `product.name.startsWith("laptop")` (theo convention DB: tên SP bắt đầu bằng category — "Laptop ...", "Điện thoại ...") → chỉ giữ SP tên bắt đầu đúng prefix. Chỉ áp dụng khi đúng 1 prefix term (skip khi so sánh 2 category) | `keyword-fallback.js` |
| **N6d-7** | ⑥.4 sort+dedup | **Tại sao:** Sau 3 lớp filter (negation + price + category), SP còn lại cần sắp xếp theo relevance. Sort matchScore desc → SP match nhiều keyword nhất lên đầu. Dedup bằng `filter(findIndex by id)` — defensive code: input từ `hybridSearch` đã unique (dedup internal), nhưng keyword fallback vẫn guard phòng edge case | `keyword-fallback.js` |
...
| **N7b** | ⑦ persistMessages | **Tại sao:** Lưu vào DB cho analytics (admin xem lịch sử chat, thống kê). Fire-and-forget: `.catch()` chỉ log warning — nếu DB chết, chatbot vẫn trả response được. Analytics là "nice to have", response cho user là "must have" | `chatbot-service.js:315` |
| **ERR-a** | catch có statusCode | **Tại sao:** AppError (400/404) là lỗi "dự kiến" do code nội bộ throw (input không hợp lệ, SP không tồn tại). Re-throw để controller trả HTTP status đúng cho client. Không persist — message lỗi không nên lưu analytics | `chatbot-service.js:323` |
| **ERR-b** | catch unknown | **Tại sao:** Lỗi "không dự kiến" (TypeError, null pointer...). Log error cho debugging + trả fallback response thay vì HTTP 500 — user vẫn thấy chatbot hoạt động. **KHÔNG persist** (khác N6d-fb) — tránh garbage trong analytics. Không update session — tránh error response làm nhiễu history | `chatbot-service.js:324-325` |
