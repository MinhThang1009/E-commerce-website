...
Thứ hai, về kiến trúc backend, hệ thống được xây dựng theo Modular Monolith với 17 module nghiệp vụ độc lập, áp dụng Dependency Injection tường minh qua factory functions, EventBus pub/sub cho các tác dụng phụ (side effects) bất đồng bộ, và UnitOfWork với SELECT FOR UPDATE để đảm bảo tính toàn vẹn dữ liệu trong các giao dịch đồng thời. Mô hình phân quyền RBAC 4 tác nhân (guest, customer, staff, admin) theo nguyên tắc đặc quyền tối thiểu (least privilege) được thực thi xuyên suốt ba tầng: middleware backend, route guard frontend và ẩn/hiện thao tác trên giao diện -- nhân viên bán hàng có toàn quyền CRUD nghiệp vụ trong khi quản trị viên độc quyền quản lý tài khoản người dùng.
