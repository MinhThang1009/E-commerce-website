...
Quản lý giỏ hàng cho cả user đã đăng nhập lẫn guest (via `sessionId` cookie). Validate stock khi thêm/cập nhật, sync cart với server, và merge guest cart vào user cart khi đăng nhập.
...
Tất cả 4 models + `sequelize` + `eventBus` + `logger` đều bắt buộc — throw Error nếu thiếu.
...
- **Merge deduplication**: Items trùng `productId + variantId` → cộng dồn quantity (cap theo maxStock). Items không trùng → append. Sau merge, guest cart status = `merged`. **Cả `getCart` inline merge lẫn `mergeCart` đều dùng `maxStock > 0 ? Math.min(newQty, maxStock) : newQty`** — khi `maxStock > 0` thì cap; khi `maxStock = 0` (hết hàng) thì giữ nguyên `newQty` để tránh zero-out item hợp lệ của user. **`mergeCart` refresh `unitPrice` về giá hiện tại** (variant.price/basePrice) — tránh stale. (Trước đây ghi nhầm field `.price` không tồn tại → refresh vô hiệu; đã sửa thành `.unitPrice`.)
...
- **`addToCart` transaction**: Toàn bộ findOrCreate cart + findOrCreate cartItem chạy trong 1 transaction. `findCartItemMatching` dùng `SELECT FOR UPDATE` (lock: transaction.LOCK.UPDATE) để serializes concurrent inserts — tránh duplicate CartItems khi 2 requests đồng thời thêm cùng product.
...
- `Cart`, `CartItem`, `Product`, `ProductVariant` models — inject từ `app.js`
