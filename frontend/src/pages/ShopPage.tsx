...
// Tùy ch?n s?p x?p s? du?c x? lý bên trong component do dùng hooks
...
const ShopPage: React.FC = () => {
  ...
  // C?p nh?t b? l?c dã ch?n khi tham s? URL thay d?i
  useEffect(() => {
    setSelectedFilters({
      categories: categoryId ? [categoryId] : [],
      brand: brandId,
  ...
  const updateFilters = (newFilters: Partial<ProductFilters>) => {
    ...
    // C?p nh?t ho?c xóa t?ng tham s? b? l?c
    ...
    // Ð?t l?i v? trang 1 khi b? l?c thay d?i
  ...
  // Xác d?nh tr?ng thái dang t?i
  const isLoading = isProductsLoading || isCategoriesLoading;

  // Chu?n b? các nhóm b? l?c cho panel l?c
  const filterGroups = [
    {
      id: 'categories',
