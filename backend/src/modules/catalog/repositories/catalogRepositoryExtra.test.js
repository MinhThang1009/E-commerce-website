...
describe('findProductVariantsByProductId — happy path', () => {
  it('gọi ProductVariant.findAll với where productId', async () => {
    ...
    const variants = [{ id: 1, productId: 5 }, { id: 2, productId: 5 }];
...
// ─── findProductsList — filter.collectionSlugsIn ─────────────────────────────

describe('findProductsList — filter.collectionSlugsIn', () => {
  it('collectionSlugsIn → include collections required:true với where slug', async () => {
    const { repo, deps } = makeRepo();
    deps.Product.findAndCountAll.mockResolvedValue({ count: 0, rows: [] });

    await repo.findProductsList({ filter: { collectionSlugsIn: ['sale', 'new'] } });
    ...
    expect(colInclude.where.slug[Op.in]).toEqual(['sale', 'new']);
  });
});
...
describe('createProductAttributes — line 759: thiếu ProductAttribute model', () => {
  it('throw khi ProductAttribute không được inject vào constructor', async () => {
    ...
    await expect(repo.createProductAttributes([{ productId: 1, name: 'Color', values: ['red'] }])).rejects.toThrow(
