...
| **N5b-2** | ⑤b Promise.all | **Tại sao:** LLM rewrite mất 3-8s, hybridSearch mất ~0.5s. Nếu chạy tuần tự (rewrite xong → mới search) tổng 3.5s. Chạy **song song** → tổng chỉ 3s. Search lần 1 (với query gốc) được dùng trong 3 trường hợp: (1) rewrite fail/timeout, (2) rewrite giống query gốc → skip search lần 2, (3) search lần 2 rỗng → fallback. Nên search lần 1 không bao giờ lãng phí | `chatbot-service.js:434` |
