...
// ─── Edge cases: default options branches ─────────────────────────────────────

describe('SequelizeCartRepository — edge cases (default options branches)', () => {
  function makeModelEdge(defaults = {}) {
    return {
      findAll: jest.fn().mockResolvedValue(defaults.findAll ?? []),
      findOne: jest.fn().mockResolvedValue(defaults.findOne ?? null),
      findByPk: jest.fn().mockResolvedValue(defaults.findByPk ?? null),
  ...
  function makeSequelizeEdge() {
    return {
      transaction: jest.fn(async (work) => work('txn')),
    };
  }

  function makeRepoEdge(overrides = {}, seqOverride = null) {
    const sequelize = seqOverride ?? makeSequelizeEdge();
    const deps = {
      Cart: makeModelEdge(),
      CartItem: makeModelEdge(),
      Product: makeModelEdge(),
      ProductVariant: makeModelEdge(),
      sequelize,
      ...overrides,
    };
    return { repo: new SequelizeCartRepository(deps), deps, sequelize };
