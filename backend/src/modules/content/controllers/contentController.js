...
class ContentController {
  ...
  getRelatedNews = async (req, res) => {
    try {
      ...
      if (news === null) return res.status(404).json({ success: false, message: 'Không tìm thấy tin tức' });
      res.json({ success: true, news });
    } catch (error) {
      res.status(500).json({ success: false, message: 'Lỗi máy chủ' });
    }
  ...
  deleteNews = async (req, res) => {
    try {
      ...
      if (!result) return res.status(404).json({ success: false, message: 'Không tìm thấy tin tức' });
