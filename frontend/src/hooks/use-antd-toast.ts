...
export const useAntdToast = () => {
  ...
  const error = (content: string, options?: ToastOptions) => showToast('error', content, options);

  const info = (content: string, options?: ToastOptions) => showToast('info', content, options);
