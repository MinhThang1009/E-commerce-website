...
// ── _canRefund — paymentStatus='refunded' (line 20) và paymentTransactionId=null + paid (line 24)

describe('PaymentService.createRefund — _canRefund branches (lines 20, 24)', () => {
  it('ném AppError 400 khi paymentStatus=refunded → line 20 true branch', async () => {
    const order = buildOrder({
      paymentStatus: 'refunded',
      paymentProvider: 'vnpay',
      paymentTransactionId: 'TX-OLD',
    });
    const repo = buildMockRepo({ findOrderByPk: jest.fn().mockResolvedValue(order) });
    const svc = buildService({ paymentRepository: repo });

    await expect(svc.createRefund({ orderId: 42, amount: 10000 })).rejects.toMatchObject({
      statusCode: 400,
    });
  });

  it('ném AppError 400 khi paid nhưng không có paymentTransactionId → line 24 true branch', async () => {
    const order = buildOrder({
      paymentStatus: 'paid',
      paymentProvider: 'vnpay',
      paymentTransactionId: null, // không có transactionId
    });
    const repo = buildMockRepo({ findOrderByPk: jest.fn().mockResolvedValue(order) });
    const svc = buildService({ paymentRepository: repo });
