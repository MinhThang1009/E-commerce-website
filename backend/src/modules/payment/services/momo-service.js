...
class MoMoService {
  constructor() {
    ...
    if (!this.partnerCode || !this.accessKey || !this.secretKey) {
      logger.warn(
        '[MoMo] MOMO_PARTNER_CODE, MOMO_ACCESS_KEY, MOMO_SECRET_KEY chưa được set — thanh toán MoMo sẽ thất bại',
      );
    }
    // Endpoint gốc: https://test-payment.momo.vn/v2/gateway/api
    this.apiEndpoint = process.env.DEV_MOMO_ENDPOINT
      ? `${process.env.DEV_MOMO_ENDPOINT}/create`
