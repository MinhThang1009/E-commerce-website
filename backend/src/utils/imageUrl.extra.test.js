...
// ─── coerceToArray — JSON.parse thành công nhưng không phải array (line 117 false branch)

describe('coerceToArray — JSON parse thành công nhưng không phải array (line 117)', () => {
  test('JSON string là object {} → false branch, dùng split fallback', () => {
    // JSON.parse('{"a":1}') = {} (object, not array) → if(Array.isArray) false → split by comma/newline
    const result = sanitizeImageCollection('{"a":1}');
    // '{"a":1}' split by comma/newline → ['{', '"a":1}'] → trimmed = ['{', '"a":1}'] → sanitize each
    // These are not valid paths but the code doesn't throw — it sanitizes them
    expect(Array.isArray(result)).toBe(true);
  });

  test('JSON string là số → false branch, dùng split', () => {
    // JSON.parse('42') = 42 (number, not array) → if(Array.isArray) false → split
    const result = sanitizeImageCollection('42');
    expect(Array.isArray(result)).toBe(true);
  });

  test('JSON string là string → false branch, dùng split', () => {
    // JSON.parse('"hello"') = 'hello' (string, not array) → Array.isArray false
    // Sau đó fallback về split: '"hello"' chia bởi comma/newline → ['"hello"'] → sanitize
    const result = sanitizeImageCollection('"hello"');
    expect(Array.isArray(result)).toBe(true);
  });
});
...
describe('imageUrl module — BACKEND_URL undefined → DEFAULT_LOCAL_BASE = "" (line 1)', () => {
  test('assetBase là "" khi tất cả env vars đều undefined', () => {
    ...
    const { assetBaseUrl, buildPublicImageUrl: buildUrl } = require('./imageUrl');
  ...
  test('API_URL không kết thúc bằng /api → apiRoot = apiUrl (lines 24-26)', () => {
    ...
    const { assetBaseUrl: base, buildPublicImageUrl: buildUrl2 } = require('./imageUrl');
