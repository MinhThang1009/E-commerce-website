...
jest.mock('@modules/ai/services/chatbot/keyword/keyword-fallback', () => ({
  simpleKeywordMatch: jest.fn(() => ({ response: 'fallback', products: [], suggestions: [], intent: 'general' })),
}));
...
describe('parseAIResponse — number mismatch (line 62)', () => {
  const products = [
    { id: 1, name: 'iPhone 14 Pro', price: 25000000, basePrice: 25000000, slug: 'iphone-14-pro', thumbnail: 'img.jpg', inStock: true, stockQuantity: 5 },
    { id: 2, name: 'iPhone 15', price: 28000000, basePrice: 28000000, slug: 'iphone-15', thumbnail: 'img2.jpg', inStock: true },
  ];
  ...
  test('không match khi LLM đề xuất sản phẩm có model number khác (line 62 — hasNumberMismatch)', () => {
    ...
    const prods = [
      { id: 3, name: 'iPhone 14 Pro', price: 22000000, basePrice: 22000000, slug: 'iphone-14-pro', thumbnail: null, inStock: true },
    ];
  ...
  test('match khi cùng số, không có version mismatch', () => {
    const prods = [
      { id: 4, name: 'iPhone 15', price: 28000000, slug: 'iphone-15', thumbnail: null, inStock: true },
    ];
    const aiText = JSON.stringify({
