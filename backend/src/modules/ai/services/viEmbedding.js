...
class VietnameseEmbeddingService {
  constructor() {
    ...
    if (this.apiKey) {
      logger.info(
        'Vietnamese Embedding Service khởi tạo thành công (multilingual-e5-large via HuggingFace)',
      );
    } else {
      logger.warn(
        'HF_API_KEY chưa được cấu hình — Vietnamese embedding sẽ fallback sang English model',
      );
    }
  }
  ...
  async generateEmbedding(text, type = 'query') {
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
      try {
        ...
        if (!isLastAttempt) {
          logger.warn(
            `VI embedding thất bại (lần ${attempt}/${maxRetries + 1}), thử lại sau ${backoffMs[attempt - 1]}ms...`,
          );
