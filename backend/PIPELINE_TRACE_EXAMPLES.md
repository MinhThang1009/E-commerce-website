...
| **N6d-7** | ⑥.4 sort+dedup | **Tại sao:** Sau 3 lớp filter (negation + price + category), SP còn lại cần sắp xếp theo relevance. Sort matchScore desc → SP match nhiều keyword nhất lên đầu. Dedup bằng `filter(findIndex by id)` loại SP trùng ID — có thể xuất hiện nếu input `products` từ hybridSearch chứa duplicate (ví dụ: cùng SP match cả vector lẫn keyword search) | `keyword-fallback.js` |
