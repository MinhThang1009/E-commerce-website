...
// ════════════════════════════════════════════════════════════════════════════════
// email.js line 165 — batch delay fires when > batchSize emails
// ════════════════════════════════════════════════════════════════════════════════

describe('email.js — batch delay fires between batches (line 165)', () => {
  it('gửi > batchSize emails → setTimeout được gọi giữa các lô (line 165)', async () => {
    jest.resetModules();
    ...
    // Start the bulk send — it will pause at the setTimeout
    const promise = emailService.sendBulkCampaignEmail(emails, 'Subject', '<p>Content</p>');

    // Process first batch (5 emails) — they are Promise.all inside loop
    ...
    // The delay setTimeout should have been called — advance timers
    jest.runAllTimers();
...
// ════════════════════════════════════════════════════════════════════════════════
// contentService.js line 238 — fire-and-forget email error handler in subscribeNewsletter
// ════════════════════════════════════════════════════════════════════════════════
...
describe('contentService.js — subscribeNewsletter fire-and-forget email error (line 238)', () => {
  it('sendNewsletterWelcomeEmail reject → logger.error được gọi (line 238)', async () => {
    ...
    const emailGateway = {
      sendNewsletterWelcomeEmail: jest.fn().mockRejectedValue(new Error('SMTP failure')),
      sendAdminFeedbackNotification: jest.fn().mockResolvedValue(),
    };

    const service = new ContentService({
      contentRepository: {
        findOrCreateSubscriber: jest.fn().mockResolvedValue({
          subscriber: { status: 'active' },
          created: true, // new subscriber → triggers welcome email
        }),
        saveSubscriber: jest.fn(),
      },
      emailGateway,
