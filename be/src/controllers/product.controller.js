  ...
  try {
    ...
    if (collection) {
      ...
      if (collectionIds.length > 0) {
        // Assume many-to-many relationship via association 'collections'
        includeConditions.push({
    ...
    const products = productsRaw.map((product) => {
      ...
      // Map category -> categories if needed
      ...
      // Map productImages -> images cho frontend tuong thich
...
const getProductById = async (req, res, next) => {
  try {
    ...
    // Fallback tÃ¬m theo slug náº¿u khÃ´ng tÃ¬m tháº¥y theo ID
    if (!product) {
    ...
    if (
      ...
      if (selectedVariant) {
        ...
        console.info(`>>> [HTTP] getProductById: id=${id}, skuId=${skuId}, queryColor=${queryColor}, effectiveColor=${variantColor}`);
...
const getProductBySlug = async (req, res, next) => {
  try {
    ...
    const { skuId } = req.query;
    console.info(`>>> [HTTP] getProductBySlug: slug=${slug}, skuId=${skuId}`);
    console.info(`>>> [HTTP] getProductBySlug: slug=${slug}, skuId=${skuId}`);
    ...
    if (productJson.variants && productJson.variants.length > 0) {
      ...
      if (selectedVariant) {
        ...
        console.info(`>>> [HTTP] getProductBySlug: slug=${slug}, skuId=${skuId}, queryColor=${queryColor}, effectiveColor=${variantColor}`);
        ...
        console.info(`[Filter Result] Total: ${productJson.images.length} -> Filtered: ${variantImages.length}`);
    ...
    const product = await Product.create(
      {
        ...
        description,
        shortDescription,
        basePrice: isVariantProduct ? 0 : price, // Set to 0 if using variants
        compareAtPrice: isVariantProduct ? null : compareAtPrice,
        images: images || [],
        thumbnail,
        // inStock Ä‘Ã£ bá» - quáº£n lÃ½ qua variants ? true : inStock, // Always true for variant products
  ...
  try {
    ...
    if (req.body.hasOwnProperty('warrantyPackageIds')) {
      ...
      if (warrantyPackageIds && warrantyPackageIds.length > 0) {
        ...
        if (warranties.length !== warrantyPackageIds.length) {
          console.log('âŒ Warranty package count mismatch!');
          throw new AppError('Má»™t hoáº·c nhiá»u gÃ³i báº£o hÃ nh khÃ´ng tá»“n táº¡i', 400);
        }
  ...
  try {
    ...
    if (relatedProductsRaw.length === 0) {
      ...
      relatedProductsRaw = await Product.findAll({
        ...
        where: {
          id: { [Op.ne]: id }, // Exclude current product
          status: 'Ðang kinh doanh', // Chá»‰ láº¥y sáº£n pháº©m Ä‘ang hoáº¡t Ä‘á»™ng
        },
        limit: parseInt(limit),
        order: [
          ['isFeatured', 'DESC'], // Æ¯u tiÃªn sáº£n pháº©m ná»•i báº­t
...
const getProductFilters = async (req, res, next) => {
  try {
    ...
    if (categoryId) {
      // Kiá»ƒm tra xem cÃ³ pháº£i lÃ  ID sá»‘ há»£p lá»‡ khÃ´ng
      ...
      if (isNumericId) {
        includeCondition.push({
          association: 'category',
          where: { id: categoryId },
          required: false, // Äáº·t required: false Ä‘á»ƒ trÃ¡nh lá»—i khi khÃ´ng tÃ¬m tháº¥y danh má»¥c
        });
      } else {
        // Náº¿u khÃ´ng pháº£i ID sá»‘, tÃ¬m theo slug
