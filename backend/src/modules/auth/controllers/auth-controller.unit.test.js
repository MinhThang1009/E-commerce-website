...
describe('AuthController._setRefreshCookie — JWT_REFRESH_EXPIRES_IN không set (line 18 fallback)', () => {
  it('dùng "7d" default khi JWT_REFRESH_EXPIRES_IN không được cấu hình', async () => {
    const saved = process.env.JWT_REFRESH_EXPIRES_IN;
    delete process.env.JWT_REFRESH_EXPIRES_IN;

    const authService = makeMockAuthService({
      login: jest.fn().mockResolvedValue({ token: 'tok', refreshToken: 'ref', user: { id: 1 } }),
    });
    const controller = new AuthController({ authService });
    ...
    // Cookie vẫn được set với maxAge từ default '7d'
    expect(res.cookie).toHaveBeenCalledWith(
      'refreshToken',
      expect.any(String),
      expect.objectContaining({ httpOnly: true }),
