...
- **Multer lưu tạm tại `uploads/temp`**: File temp bị xóa sau khi processed. Nếu process thất bại → temp file có thể leak. `cleanupOrphanedFiles` **không dọn** `temp/` và `images/thumbnails/` — phải dọn thủ công nếu cần (thumbnails là derivative files, không có DB record riêng).
- **`cleanupOrphanedFiles` skip thumbnails và temp**: Thumbnails không có `Image` DB record riêng → cleanup **bỏ qua** `images/thumbnails/` (tránh xóa nhầm). Temp files cũng được bỏ qua. Chỉ xóa file trong `images/{category}/` không có DB record active.
- **Thumbnail path có year/month**: `generateThumbnails()` lưu tại `images/thumbnails/{year}/{month}/{uuid}_{size}.ext`. `deleteImage()` dùng `image.createdAt` để reconstruct path đúng — KHÔNG hardcode `images/thumbnails/{uuid}_{size}` (thiếu year/month).
- **`convertBase64ToFile` strip EXIF**: Gọi `sharp(buffer).rotate().withMetadata(false).toFile()` — KHÔNG dùng `fs.writeFile` trực tiếp. Đảm bảo ảnh base64 không leak GPS location.
