 ...
 */
// Catalog Controller — Sprint 6a: Category + Brand handlers.
...
class CatalogController {
  ...
  getProductsByCategory = async (req, res, next) => {
    try {
      const data = await this.catalogService.getProductsByCategory({
        id: req.params.id,
        ...req.query,
      });
      res.status(200).json({ status: 'success', data });
  ...
  getProductsByBrand = async (req, res, next) => {
    try {
      const data = await this.catalogService.getProductsByBrand({
  ...
  getRelatedProducts = async (req, res, next) => {
    try {
      const data = await this.catalogService.getRelatedProducts({
        id: req.params.id,
        ...req.query,
      });
      res.status(200).json({ status: 'success', data });
