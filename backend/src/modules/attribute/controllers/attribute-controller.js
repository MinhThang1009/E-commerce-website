...
const getProductAttributeGroups = async (req, res, next) => {
  try {
    res.json({ status: 'success', data: await service.getProductAttributeGroups(req.params.productId) });
  } catch (error) { next(error); }
};
...
const addAttributeValue = async (req, res, next) => {
  try {
    const data = await service.addValue({ ...req.body, attributeGroupId: req.params.attributeGroupId });
    res.status(201).json({ status: 'success', data, message: 'Thêm giá trị thuộc tính thành công' });
  } catch (error) { next(error); }
};
...
const assignAttributeGroupToProduct = async (req, res, next) => {
  try {
    ...
    res.status(201).json({ status: 'success', data, message: 'Gán nhóm thuộc tính cho sản phẩm thành công' });
  } catch (error) { next(error); }
};
...
const previewProductName = async (req, res, next) => {
  try {
    ...
    if (!baseName) return res.status(400).json({ status: 'error', message: 'Tên cơ sở là bắt buộc' });
    const data = await service.previewProductName(baseName, selectedAttributes || [], { separator: separator || ' ', includeDetails: includeDetails || false });
...
const getNameAffectingAttributes = async (req, res, next) => {
  try {
    ...
    res.json({ status: 'success', data, message: 'Lấy danh sách thuộc tính ảnh hưởng đến tên thành công' });
  } catch (error) { next(error); }
};
...
const batchGenerateProductNames = async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(items)) return res.status(400).json({ status: 'error', message: 'Tham số items phải là một mảng' });
...
module.exports = {
  getAttributeGroups, getProductAttributeGroups,
  ...
  previewProductName, getNameAffectingAttributes, batchGenerateProductNames, generateNameRealTime,
};
