...
const repo = require('./sequelize-discount-code-repository');
...
describe('discountCodeRepository', () => {
  ...
  describe('findAll', () => {
    ...
    it('dùng where={} mặc định khi không truyền where — default branch (line 24)', async () => {
      DiscountCode.findAndCountAll.mockResolvedValue({ count: 0, rows: [] });
      // Không truyền `where` → default {} được dùng
      await repo.findAll({ limit: 5, offset: 0 });
