...
describe('VietnameseEmbeddingService.isAvailable', () => {
  ...
  test('trả true khi HF_API_KEY được cấu hình', () => {
    ...
    const service = require('./vi-embedding');
...
// ════════════════════════════════════════════════════════════════════════════
// generateEmbedding — type='passage' branch (line 38)
// ════════════════════════════════════════════════════════════════════════════

describe('VietnameseEmbeddingService.generateEmbedding — passage type', () => {
  let service;
  ...
  beforeEach(() => {
    ...
    mockAxiosPost.mockReset();
    jest.mock('axios', () => ({ post: (...args) => mockAxiosPost(...args) }));
    process.env.HF_API_KEY = 'hf-passage-key';
    service = require('./vi-embedding');
  });
  ...
  test('type=passage → dùng prefix "passage: " thay vì "query: " (line 38)', async () => {
    ...
    await service.generateEmbedding('sản phẩm công nghệ', 'passage');

    expect(mockAxiosPost).toHaveBeenCalledWith(
      expect.any(String),
      { inputs: 'passage: sản phẩm công nghệ' },
      expect.any(Object),
    );
  });
});
