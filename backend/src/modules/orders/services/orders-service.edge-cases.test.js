...
// ─── confirmReceived — line 763: pointsEarned falsy với status=delivered ──────

describe('confirmReceived — status=delivered, pointsEarned=0 → (0||0)!==0=false, alreadyProcessed=false', () => {
  it('tiếp tục xử lý bình thường khi pointsEarned=0 dù status=delivered (|| 0 right branch)', async () => {
    // order.pointsEarned = 0 (falsy) → (0 || 0) = 0 → 0 !== 0 = false → alreadyProcessed = false
    // _canConfirmReceived('delivered') = true → không throw
    const { service, repo, eventBus } = buildService();
    const order = {
      id: 20,
      status: 'delivered',
    ...
    const result = await service.confirmReceived({ id: 20, userId: 1 });
...
// ─── confirmReceived — alreadyProcessed=true (line 763/777) ──────────────────

describe('confirmReceived — alreadyProcessed=true → return early (idempotent)', () => {
  it('trả về alreadyConfirmed khi status=delivered và pointsEarned > 0', async () => {
    // order.status === 'delivered' && pointsEarned !== 0 → alreadyProcessed = true
    // Phủ nhánh true của (order.pointsEarned || 0) !== 0 và if(alreadyProcessed) at line 777
    const { service, repo } = buildService();
    const order = {
      id: 10,
      status: 'delivered',
    ...
    const result = await service.confirmReceived({ id: 10, userId: 1 });

    expect(result.message).toBe('orders.alreadyConfirmed');
    expect(result.pointsEarned).toBe(0);
    // Không trao điểm vì đã xử lý
    expect(repo.findUserById).not.toHaveBeenCalled();
  });
});

// ─── getUserOrders — line 520: unitPrice không parse được → || 0 ─────────────

describe('getUserOrders — unitPrice non-numeric → parseFloat(unitPrice) || 0', () => {
  it('gán price = 0 khi unitPrice là null (parseFloat(null)=NaN → || 0)', async () => {
    const { service, repo } = buildService();
    const row = {
      toJSON: () => ({
        id: 1,
        items: [
          {
            unitPrice: null, // undefined check: null !== undefined → true; price undefined
            // parseFloat(null) = NaN → NaN || 0 = 0
          },
        ],
      }),
    };
