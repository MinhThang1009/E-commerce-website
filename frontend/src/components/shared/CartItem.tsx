...
interface CartItemProps {
  ...
  isCheckout?: boolean;
  readonly?: boolean;
  maxStock?: number; // tồn kho thực tế từ validateCart
}
...
const CartItem: React.FC<CartItemProps> = ({ item, isCheckout = false, readonly = false, maxStock }) => {
  ...
  const handleQuantityChange = async (newQuantity: number) => {
    ...
    try {
      // Luôn thử API server trước (hoạt động cho cả khách và người dùng đã đăng nhập)
