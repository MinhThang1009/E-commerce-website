...
// ═══════════════════════════════════════════════════════════════════════════
// createProduct — isVariantProduct, defaults, transaction args (L548-L640)
// ═══════════════════════════════════════════════════════════════════════════

describe('createProduct — isVariantProduct và basePrice (L548)', () => {
  it('có variants → isVariantProduct = true, basePrice = 0 (L548)', async () => {
    const { service, catalogRepository } = makeService();
    ...
    await service.createProduct({
      payload: {
        name: 'VP',
        price: 50000,
        variants: [{ sku: 'V1', price: 10000, stockQuantity: 5 }],
      },
    });

    expect(catalogRepository.createProduct).toHaveBeenCalledWith(
      expect.objectContaining({ isVariantProduct: true, basePrice: 0 }),
      expect.any(Object),
    );
  });

  it('không có variants → isVariantProduct = false, basePrice = price (L548)', async () => {
    const { service, catalogRepository } = makeService();
    ...
    await service.createProduct({
      payload: { name: 'P', price: 50000 },
    });

    expect(catalogRepository.createProduct).toHaveBeenCalledWith(
      expect.objectContaining({ isVariantProduct: false, basePrice: 50000 }),
      expect.any(Object),
    );
  });

  it('status mặc định = "active" khi không truyền (L562)', async () => {
    const { service, catalogRepository } = makeService();
    ...
    await service.createProduct({ payload: { name: 'P' } });
  ...
  it('tags mặc định = [] khi không truyền (L564)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 4 });
  ...
  it('seoKeywords mặc định = [] khi không truyền (L568)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 5 });
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(makeProductRow({ id: 5 }));

    await service.createProduct({ payload: { name: 'P' } });

    expect(catalogRepository.createProduct).toHaveBeenCalledWith(
      expect.objectContaining({ seoKeywords: [] }),
      expect.any(Object),
    );
  });

  it('specifications mặc định = {} khi không truyền (L570)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 6 });
  ...
  it('throw 409 khi tên sản phẩm đã tồn tại (L572)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.findProductByName.mockResolvedValue({ id: 99, name: 'Existing' });

    await expect(service.createProduct({ payload: { name: 'Existing' } })).rejects.toMatchObject({
      statusCode: 409,
      message: 'catalog.productNameExists',
    });
  });

  it('throw 400 với message catalog.categoriesNotExist khi categories không đủ (L580)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 7 });
    catalogRepository.findCategoriesByIds.mockResolvedValue([{ id: 1 }]); // chỉ 1, nhưng request 2

    await expect(
      service.createProduct({
        payload: { name: 'P', categoryIds: [1, 2] },
      }),
    ).rejects.toMatchObject({ message: 'catalog.categoriesNotExist' });
  });
});

describe('createProduct — variant fields đúng (L630-L637)', () => {
  it('variant.price = parseFloat → 0 khi null (L630)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 10 });
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(makeProductRow({ id: 10 }));

    await service.createProduct({
      payload: {
        name: 'P',
        variants: [{ sku: 'V1', price: null, stockQuantity: 5 }],
      },
    });

    const variantsArg = catalogRepository.createProductVariants.mock.calls[0][0];
    expect(variantsArg[0].price).toBe(0);
  });

  it('variant.stockQuantity được parse thành int (L632)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 11 });
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(makeProductRow({ id: 11 }));

    await service.createProduct({
      payload: {
        name: 'P',
        variants: [{ sku: 'V1', price: 5000, stockQuantity: '10' }],
      },
    });

    const variantsArg = catalogRepository.createProductVariants.mock.calls[0][0];
    expect(variantsArg[0].stockQuantity).toBe(10);
  });

  it('variant.isDefault = true cho variant đầu tiên khi không set (L633: i === 0)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 12 });
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(makeProductRow({ id: 12 }));

    await service.createProduct({
      payload: {
        name: 'P',
        variants: [
          { sku: 'V1', price: 5000 }, // i=0, không có isDefault → true
          { sku: 'V2', price: 6000 }, // i=1 → false
        ],
      },
    });

    const variantsArg = catalogRepository.createProductVariants.mock.calls[0][0];
    expect(variantsArg[0].isDefault).toBe(true); // i === 0
    expect(variantsArg[1].isDefault).toBe(false); // i !== 0, no isDefault set
  });

  it('variant.isAvailable = true mặc định (L634: !== false)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 13 });
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(makeProductRow({ id: 13 }));

    await service.createProduct({
      payload: { name: 'P', variants: [{ sku: 'V1', price: 5000 }] },
    });

    const variantsArg = catalogRepository.createProductVariants.mock.calls[0][0];
    expect(variantsArg[0].isAvailable).toBe(true);
  });

  it('variant.isAvailable = false khi truyền false (L634)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 14 });
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(makeProductRow({ id: 14 }));

    await service.createProduct({
      payload: { name: 'P', variants: [{ sku: 'V1', price: 5000, isAvailable: false }] },
    });

    const variantsArg = catalogRepository.createProductVariants.mock.calls[0][0];
    expect(variantsArg[0].isAvailable).toBe(false);
  });

  it('variant.attributes mặc định = {} khi không có (L635)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 15 });
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(makeProductRow({ id: 15 }));
    ...
    const variantsArg = catalogRepository.createProductVariants.mock.calls[0][0];
    expect(variantsArg[0].attributes).toEqual({});
  });

  it('variant.sortOrder = i khi không set (L637)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 16 });
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(makeProductRow({ id: 16 }));

    await service.createProduct({
      payload: {
        name: 'P',
        variants: [
          { sku: 'V0', price: 5000 }, // i=0
          { sku: 'V1', price: 6000 }, // i=1
        ],
      },
    });

    const variantsArg = catalogRepository.createProductVariants.mock.calls[0][0];
    expect(variantsArg[0].sortOrder).toBe(0);
    expect(variantsArg[1].sortOrder).toBe(1);
  });
});

describe('createProduct — isThumbnail cho variant images (L749)', () => {
  it('ảnh đầu tiên của variant → isThumbnail = true (j===0), ảnh thứ 2 → false (L749)', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.createProduct.mockResolvedValue({ id: 20, slug: 'p' });
    catalogRepository.createProductVariants.mockResolvedValue([{ id: 100 }]);
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(makeProductRow({ id: 20 }));

    await service.createProduct({
      payload: {
        name: 'P',
        variants: [{ sku: 'V1', price: 1000, images: ['v1img1.jpg', 'v1img2.jpg', 'v1img3.jpg'] }],
      },
    });

    const imageRows = catalogRepository.createProductImages.mock.calls[0][0];
    expect(imageRows[0].isThumbnail).toBe(true); // j=0
    expect(imageRows[1].isThumbnail).toBe(false); // j=1
    expect(imageRows[2].isThumbnail).toBe(false); // j=2
  });
});

describe('updateProduct — variant images trong updateProduct (L741, L749, L755)', () => {
  it('chỉ tạo variant images khi v.images có phần tử (L741: v.images && v.images.length > 0)', async () => {
    const { service, catalogRepository } = makeService();
    const product = makeProductRow({ id: 1 });
    catalogRepository.findProductByPk.mockResolvedValue(product);
    catalogRepository.createProductVariants.mockResolvedValue([{ id: 50 }]);
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(product);

    await service.updateProduct({
      id: 1,
      patch: { variants: [{ sku: 'V1', price: 5000, images: ['img1.jpg'] }] },
    });

    // Phải gọi createProductImages cho variant images
    expect(catalogRepository.createProductImages).toHaveBeenCalledWith(
      expect.arrayContaining([
        expect.objectContaining({ variantId: 50, imageUrl: 'img1.jpg', isThumbnail: true }),
      ]),
      expect.any(Object),
    );
  });

  it('không tạo variant images khi v.images rỗng (L741: v.images.length > 0 false)', async () => {
    const { service, catalogRepository } = makeService();
    const product = makeProductRow({ id: 2 });
    catalogRepository.findProductByPk.mockResolvedValue(product);
    catalogRepository.createProductVariants.mockResolvedValue([{ id: 51 }]);
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(product);

    await service.updateProduct({
      id: 2,
      patch: { variants: [{ sku: 'V2', price: 5000, images: [] }] },
    });

    // createProductImages không được gọi vì images rỗng
    expect(catalogRepository.createProductImages).not.toHaveBeenCalled();
  });

  it('clearProductImages được gọi với "variants" khi patch có variants (L727)', async () => {
    const { service, catalogRepository } = makeService();
    const product = makeProductRow({ id: 3 });
    catalogRepository.findProductByPk.mockResolvedValue(product);
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(product);
    ...
    expect(catalogRepository.clearProductImages).toHaveBeenCalledWith(
      3,
      'variants',
      expect.any(Object),
    );
  ...
  it('clearProductVariants được gọi khi patch có variants (L728)', async () => {
    const { service, catalogRepository } = makeService();
    const product = makeProductRow({ id: 4 });
    catalogRepository.findProductByPk.mockResolvedValue(product);
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(product);
    ...
    expect(catalogRepository.clearProductVariants).toHaveBeenCalledWith(4, expect.any(Object));
  });
});

describe('deleteProduct — error key đúng (L766)', () => {
  it('throw catalog.productNotFound khi không tìm thấy (L766)', async () => {
    const { service } = makeService();
    await expect(service.deleteProduct({ id: 99 })).rejects.toMatchObject({
...
// ═══════════════════════════════════════════════════════════════════════════
// updateProduct — categoryIds và transaction args (L697, L700)
// ═══════════════════════════════════════════════════════════════════════════

describe('updateProduct — categoryIds handling (L697, L700)', () => {
  it('không gọi setProductCategories khi patch không có categoryIds (L697)', async () => {
    const { service, catalogRepository } = makeService();
    const product = makeProductRow({ id: 1 });
    catalogRepository.findProductByPk.mockResolvedValue(product);
    catalogRepository.findProductByIdWithFullDetails.mockResolvedValue(product);
    ...
    expect(catalogRepository.setProductCategories).not.toHaveBeenCalled();
  });

  it('throw catalog.categoriesNotExist khi update categoryIds không tồn tại đủ (L700)', async () => {
    const { service, catalogRepository } = makeService();
    const product = makeProductRow({ id: 1 });
    catalogRepository.findProductByPk.mockResolvedValue(product);
    catalogRepository.findCategoriesByIds.mockResolvedValue([]); // không tìm thấy

    await expect(
      service.updateProduct({ id: 1, patch: { categoryIds: [1, 2] } }),
