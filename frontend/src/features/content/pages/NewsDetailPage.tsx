...
import { useGetNewsBySlugQuery, useGetNewsQuery, useGetRelatedNewsQuery } from '../api/news-api';
  ...
  return (
    <>
      {data.news.map((n) => (
        <Link
          key={n.id}
          to={buildRoute.newsDetail(n.slug)}
  ...
  if (!newsItem)
    return (
      <div className="min-h-screen flex flex-col justify-center items-center">
        ...
        <Link to={ROUTES.NEWS} className="text-primary-600 hover:underline">
          {t('news.backToList')}
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-white dark:bg-neutral-900 pt-28 pb-20">
      <div className="container mx-auto px-4">
        <nav className="mb-8 text-sm text-neutral-500 font-medium overflow-x-auto whitespace-nowrap no-scrollbar">
          ...
          <Link to={ROUTES.NEWS} className="hover:text-primary-600">
            {t('news.newsLink')}
          </Link>
          <span className="mx-2">/</span>
            ...
            <header className="mb-10">
              ...
              <div className="flex items-center gap-4 py-6 border-y border-neutral-100 dark:border-neutral-800">
                ...
                <div>
                  ...
                  <div className="text-neutral-500 text-xs">{t('news.authorAt')}</div>
                </div>
              </div>
            </header>
