...
- **Dùng `Read` với `offset` + `limit`** — không đọc file >150 dòng/lần; nếu cần toàn bộ file, đọc từng đoạn 100-150 dòng
- **Tìm symbol cụ thể** với `Grep pattern` thay vì đọc toàn bộ file rồi tìm

### 2. Quy trình sửa từng file
```
Grep (tìm vị trí) → Read offset/limit (đọc đoạn cụ thể) → Edit (sửa) → Verify (grep lại)
```
