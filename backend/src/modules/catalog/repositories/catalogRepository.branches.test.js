...
const SequelizeCatalogRepository = require('./SequelizeCatalogRepository');
...
// ════════════════════════════════════════════════════════════════════════════
// findProductsByCollectionId — lines 206-233
// Nhánh: gọi với đúng options và gọi không có options (default params)
// ════════════════════════════════════════════════════════════════════════════

describe('findProductsByCollectionId', () => {
  it('gọi không có options → sử dụng default sort/order', async () => {
    const { repo, deps } = makeRepo();
    deps.Product.findAndCountAll.mockResolvedValue({ count: 0, rows: [] });

    await repo.findProductsByCollectionId(10);

    const call = deps.Product.findAndCountAll.mock.calls[0][0];
    expect(call.order).toEqual([['createdAt', 'DESC']]);
    expect(call.limit).toBeUndefined();
    expect(call.offset).toBeUndefined();
    // Status active vẫn được áp dụng
    expect(call.where).toEqual({ status: 'active' });
  });

  it('gọi với sort tùy chỉnh → order được override', async () => {
    const { repo, deps } = makeRepo();
    deps.Product.findAndCountAll.mockResolvedValue({ count: 3, rows: [] });

    await repo.findProductsByCollectionId(5, { sort: 'basePrice', order: 'ASC', limit: 20, offset: 0 });
...
describe('findProductsList — gọi không có tham số', () => {
  ...
  it('findProductsList không có collectionIdsIn lẫn collectionSlugsIn → không thêm collections required', async () => {
    const { repo, deps } = makeRepo();
    deps.Product.findAndCountAll.mockResolvedValue({ count: 0, rows: [] });
    ...
    const call = deps.Product.findAndCountAll.mock.calls[0][0];
    // Không có collections include nào có required:true
    const requiredCol = call.include.find(
      (inc) => inc.association === 'collections' && inc.required === true,
    );
    expect(requiredCol).toBeUndefined();
  });
});
...
// ════════════════════════════════════════════════════════════════════════════
// setCollectionProducts — nhánh productIds là null/undefined
// ════════════════════════════════════════════════════════════════════════════

describe('setCollectionProducts — nhánh productIds null', () => {
  it('productIds = null → chỉ destroy, không bulkCreate', async () => {
    const { repo, deps } = makeRepo();

    await repo.setCollectionProducts(3, null);

    expect(deps.ProductCollection.destroy).toHaveBeenCalled();
    expect(deps.ProductCollection.bulkCreate).not.toHaveBeenCalled();
  });

  it('productIds = undefined → chỉ destroy, không bulkCreate', async () => {
    const { repo, deps } = makeRepo();

    await repo.setCollectionProducts(4, undefined);
...
// ════════════════════════════════════════════════════════════════════════════
// findAllCollections — filter.isActive = false (truthy condition với false value)
// ════════════════════════════════════════════════════════════════════════════

describe('findAllCollections — filter.isActive = false', () => {
  it('filter.isActive = false → where.isActive = false (không bị bỏ qua)', async () => {
    const { repo, deps } = makeRepo();
    deps.Collection.findAll.mockResolvedValue([]);

    await repo.findAllCollections({ filter: { isActive: false } });
...
// ════════════════════════════════════════════════════════════════════════════
// destroyCollectionProducts — line 206: gọi không có options (default = {})
// ════════════════════════════════════════════════════════════════════════════

describe('destroyCollectionProducts — line 206: gọi không có options (default param {})', () => {
  it('gọi không truyền options → ProductCollection.destroy với where chỉ có collectionId', async () => {
    // Line 206: options = {} (default) → destroy chỉ có where
    const { repo, deps } = makeRepo();

    await repo.destroyCollectionProducts(7);
