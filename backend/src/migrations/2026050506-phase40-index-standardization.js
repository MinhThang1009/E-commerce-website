...
const AUDIT_INDEX_RENAMES = [
  ...
  ['audit_logs', 'audit_logs_entity_type_entity_id',   'idx_audit_logs_entity',     'entity_type, entity_id'],
];
...
module.exports = {
  async up(queryInterface) {
    ...
    // 2. Drop duplicate UNIQUE on product_variants.sku (giữ idx_product_variants_sku → rename uq)
    {
      ...
      const namedExists = await indexExists(queryInterface, 'product_variants', 'idx_product_variants_sku');
      const finalExists = await indexExists(queryInterface, 'product_variants', 'uq_product_variants_sku');
      if (!finalExists) {
  ...
  async down(queryInterface) {
    ...
    for (const [table, indexName] of NEW_INDEXES) {
      const exists = await indexExists(queryInterface, table, indexName);
      if (exists) {
        await queryInterface.sequelize.query(`ALTER TABLE \`${table}\` DROP INDEX \`${indexName}\``);
      }
    }
    for (const [table, oldName, newName, cols] of AUDIT_INDEX_RENAMES) {
      const newExists = await indexExists(queryInterface, table, newName);
