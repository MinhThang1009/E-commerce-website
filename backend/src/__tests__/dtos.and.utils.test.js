...
describe('toCollectionDto', () => {
  test('trả về null khi input là null', () => {
    expect(toCollectionDto(null)).toBeNull();
  });

  test('trả về null khi input là undefined', () => {
    expect(toCollectionDto(undefined)).toBeNull();
  });

  test('pass-through object hợp lệ', () => {
    const col = { id: 3, name: 'Sale 2024' };
    expect(toCollectionDto(col)).toBe(col);
  });
});
...
describe('lockRow', () => {
  test('throw khi không truyền transaction', async () => {
    ...
    await expect(lockRow(model, { id: 1 }, null)).rejects.toThrow(
...
describe('SequelizeInventoryRepository', () => {
  ...
  describe('findInventoryLogs', () => {
    test('gọi findAndCountAll với order DESC và include Product, ProductVariant, User', async () => {
      ...
      const result = await repo.findInventoryLogs({ where: { productId: 2 }, limit: 10, offset: 0 });
