...
describe('validateVariantAttributes', () => {
  test('Variant value nằm trong product values → true', () => {
    expect(validateVariantAttributes(
...
describe('generateVariantSku', () => {
  test('Concat productSku + uppercase attributes joined "-"', () => {
    expect(generateVariantSku('IPHONE17', { color: 'black', storage: '256gb' }))
      .toBe('IPHONE17-BLACK-256GB');
  });

  test('Strip whitespace trong attribute value', () => {
    expect(generateVariantSku('SKU', { color: 'dark blue' }))
      .toBe('SKU-DARKBLUE');
  });

  test('Multiple spaces collapsed', () => {
    expect(generateVariantSku('X', { config: '32 GB DDR5' }))
      .toBe('X-32GBDDR5');
  });
});
