        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="h-80 bg-neutral-100 dark:bg-neutral-800 rounded-3xl animate-pulse"
              />
            ))}
          </div>
        ) : error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {collections.map(
              // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Collection API response
              (collection: any) => (
                <Link
                  key={collection.id}
