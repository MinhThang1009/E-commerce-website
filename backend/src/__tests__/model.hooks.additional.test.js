...
describe('brand.js — name virtual field getter/setter', () => {
  ...
  beforeAll(() => {
    const { fields } = loadModelCapture('../models/brand');
...
describe('brand.js — beforeValidate hook', () => {
  ...
  beforeAll(() => {
    ...
    const { hooks } = loadModelCapture('../models/brand');
...
describe('category.js — name virtual field getter/setter', () => {
  ...
  beforeAll(() => {
    const { fields } = loadModelCapture('../models/category');
    fieldDef = fields.name;
  });
...
describe('category.js — beforeValidate hook', () => {
  ...
  beforeAll(() => {
    const { hooks } = loadModelCapture('../models/category');
    hook = hooks.beforeValidate;
  });
...
describe('collection.js — name virtual field getter/setter', () => {
  let fieldDef;

  beforeAll(() => {
    const { fields } = loadModelCapture('../models/collection');
    fieldDef = fields.name;
  });

  it('getter should return the value of nameVi', () => {
    const inst = makeInstance({ nameVi: 'Hàng mới về' });
    expect(fieldDef.get.call(inst)).toBe('Hàng mới về');
  });

  it('setter should store the value in nameVi', () => {
    const inst = makeInstance({ nameVi: null });
    fieldDef.set.call(inst, 'Flash Sale');
    expect(inst.getDataValue('nameVi')).toBe('Flash Sale');
  });
});

describe('collection.js — description virtual field getter/setter', () => {
  let fieldDef;
  ...
  it('getter should return the value of descriptionVi', () => {
    const inst = makeInstance({ descriptionVi: 'Bộ sưu tập mùa hè' });
    expect(fieldDef.get.call(inst)).toBe('Bộ sưu tập mùa hè');
  });

  it('setter should store the value in descriptionVi', () => {
    const inst = makeInstance({ descriptionVi: null });
    fieldDef.set.call(inst, 'Ưu đãi mới');
    expect(inst.getDataValue('descriptionVi')).toBe('Ưu đãi mới');
  });
});

describe('collection.js — beforeValidate hook', () => {
  let hook;

  beforeAll(() => {
    const { hooks } = loadModelCapture('../models/collection');
    hook = hooks.beforeValidate;
  });

  it('should generate slug from name when slug is empty', () => {
    const collection = { name: 'Bán chạy', slug: null };
    hook(collection);
    expect(collection.slug).toMatch(/^[a-z0-9-]+$/);
  });

  it('should NOT overwrite an existing slug', () => {
    // collection.js: if (collection.name && !collection.slug)
    const collection = { name: 'Sale', slug: 'sale-existing' };
    hook(collection);
    expect(collection.slug).toBe('sale-existing');
  });

  it('should NOT set slug when name is null', () => {
    const collection = { name: null, slug: null };
    hook(collection);
    expect(collection.slug).toBeNull();
  });

  it('should NOT set slug when name is empty string', () => {
    const collection = { name: '', slug: null };
    hook(collection);
    expect(collection.slug).toBeNull();
  });

  it('should produce lowercase hyphenated slug', () => {
    const collection = { name: 'New Arrivals', slug: null };
    hook(collection);
    expect(collection.slug).toBe('new-arrivals');
  });
});
...
describe('productVariant.js — attributes getter', () => {
  ...
  beforeAll(() => {
    const { fields } = loadModelCapture('../models/productVariant');
...
function loadUserModel({ argon2Hash, argon2Verify, bcryptCompare } = {}) {
  let capturedHooks = {};
  let UserModel;

  // Create jest.fn() instances outside isolateModules so they can be referenced
  // later in assertions. These are the actual mock implementations.
  const mockHash = jest.fn().mockResolvedValue(argon2Hash || '$argon2id$hashed');
  const mockVerify = jest.fn().mockResolvedValue(argon2Verify !== undefined ? argon2Verify : true);
  const mockBcryptCompare = jest.fn().mockResolvedValue(bcryptCompare !== undefined ? bcryptCompare : true);

  jest.isolateModules(() => {
    // jest.doMock is NOT hoisted — safe to reference outer variables.
    jest.doMock('argon2', () => ({
      hash: mockHash,
      verify: mockVerify,
      argon2id: 'argon2id',
    ...
    // Must return a function/constructor so .prototype assignment in user.js works.
...
describe('user.js — beforeCreate hook', () => {
  ...
  it('should hash password with argon2id when password is provided', async () => {
    const user = { password: 'plaintext-password' };
    await hook(user);
    expect(mockHash).toHaveBeenCalledWith('plaintext-password', {
      type: 'argon2id',
    });
    expect(user.password).toBe('$argon2id$hashed');
  });
...
describe('user.js — beforeUpdate hook', () => {
  ...
  beforeAll(() => {
    const result = loadUserModel({ argon2Hash: '$argon2id$updated' });
  ...
  it('should hash password when changed() returns true for password field', async () => {
    ...
    await hook(user);
    expect(mockHash).toHaveBeenCalledWith('new-plain-password', {
      type: 'argon2id',
    });
    expect(user.password).toBe('$argon2id$updated');
  });

  it('should NOT hash password when changed() returns false for password field', async () => {
    mockHash.mockClear();
    const user = {
      password: '$argon2id$already-hashed',
      changed: () => false,
    };
    await hook(user);
    expect(mockHash).not.toHaveBeenCalled();
    expect(user.password).toBe('$argon2id$already-hashed');
  });
});
...
describe('user.js — User.prototype.comparePassword', () => {
  ...
  it('should use argon2.verify when password hash starts with $argon2', async () => {
    const userContext = { password: '$argon2id$v=19$m=65536,t=3,p=4$somesalt$somehash' };
    const result = await UserModel.prototype.comparePassword.call(userContext, 'candidate');
    expect(mockVerify).toHaveBeenCalledWith(userContext.password, 'candidate');
    expect(result).toBe(true);
  });

  it('should use bcrypt.compare when password hash is a bcrypt hash', async () => {
    const userContext = { password: '$2b$10$somebcrypthash' };
    const result = await UserModel.prototype.comparePassword.call(userContext, 'candidate');
...
describe('attributeValue.js — colorCode isValidColor validator', () => {
  ...
  beforeAll(() => {
    const { fields } = loadModelCapture('../models/attributeValue');
