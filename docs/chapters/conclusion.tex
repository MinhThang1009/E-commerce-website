...
Dự án đã hoàn thành mục tiêu đề ra là xây dựng hệ thống website thương mại điện tử TechStore chuyên bán thiết bị công nghệ, tích hợp AI Chatbot sử dụng kỹ thuật RAG để tư vấn sản phẩm theo thời gian thực.

Về ba câu hỏi nghiên cứu đặt ra ở Chương~1: (1) RAG Hybrid cho kết quả chính xác 90\% (18/20 kịch bản), vượt trội so với tìm kiếm đơn lẻ nhờ overlap boost giữa dense và sparse retrieval -- tên model cụ thể đạt 4/4 nhờ sparse, truy vấn ngữ nghĩa đạt 5/6 nhờ dense; (2) bước chuẩn hóa 71 mẫu regex xử lý thành công 4/4 kịch bản viết tắt với chi phí dưới 1ms; (3) Modular Monolith với UnitOfWork đảm bảo ACID trong giao dịch đồng thời và cho phép kiểm thử độc lập từng module (5.381 unit tests, coverage 100\% statements).

Những kết quả chính đạt được gồm năm điểm sau.

Thứ nhất, về thành phần trọng tâm là chatbot AI RAG, dự án đã nghiên cứu và cài đặt thành công pipeline RAG Hybrid gồm 7 bước với điểm đặc trưng là tìm kiếm kết hợp giữa cosine similarity (dense retrieval, ngưỡng 0.45) và BM25-inspired keyword scoring (sparse retrieval, trọng số tên sản phẩm nhân 3) cùng overlap boost +0.05. Chiến lược chain fallback qua ba nhà cung cấp embedding (Jina v3, HuggingFace e5-large-instruct, HuggingFace e5-large), tất cả tạo vector 1024 chiều, đảm bảo tính sẵn sàng cao. Pipeline hoạt động tốt cho các câu hỏi tìm kiếm ngữ nghĩa, tên model cụ thể và câu hỏi viết tắt tiếng Việt.

Thứ hai, về kiến trúc backend, hệ thống được xây dựng theo Modular Monolith với 17 module nghiệp vụ độc lập, áp dụng Dependency Injection tường minh qua factory functions, EventBus pub/sub cho side effects bất đồng bộ, và UnitOfWork với SELECT FOR UPDATE để đảm bảo tính toàn vẹn dữ liệu trong các giao dịch đồng thời. Mô hình phân quyền RBAC 4 tác nhân (guest, customer, staff, admin) theo nguyên tắc least privilege được thực thi xuyên suốt ba tầng: middleware backend, route guard frontend và ẩn/hiện thao tác trên giao diện -- nhân viên bán hàng có toàn quyền CRUD nghiệp vụ, trong khi quản trị viên chỉ xem dữ liệu nghiệp vụ và độc quyền quản lý tài khoản người dùng.
...
Thứ năm, về chất lượng phần mềm, bộ kiểm thử gồm $\sim$7.303 test cases phân bố trên 5 tầng (unit 5.381, integration 210, API HTTP 675, E2E 100, FE component 937) với coverage 100\% statements và lines, 99,91\% branches của unit tests. Bổ sung mutation testing (Stryker, ngưỡng 70\%) và property-based testing (fast-check, 25 business invariants) đảm bảo chất lượng assertion vượt coverage đơn thuần. CI/CD pipeline tự động hóa quy trình kiểm tra, Husky pre-commit hooks kiểm tra kiến trúc và phát hiện secret key trước mỗi commit.
