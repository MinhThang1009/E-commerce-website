...
describe('POST /api/products (admin)', () => {
  test('admin tạo sản phẩm với payload đầy đủ → 201', async () => {
    ...
    const createdId = res.body?.data?.id || res.body?.data?.product?.id;
    if (createdId) {
      const { ProductVariant } = require('@models');
      await ProductVariant.destroy({ where: { productId: createdId }, force: true }).catch(
        () => {},
      );
      const { Product } = require('@models');
      await Product.destroy({ where: { id: createdId }, force: true }).catch(() => {});
    }
  });
  ...
  test('không auth → 401', async () => {
    const res = await request(app)
      .post('/api/products')
      .send({ name: 'NoAuth', description: 'x', shortDescription: 'x', price: 100 });
    expect(res.status).toBe(401);
  });
  ...
  test('không phải admin → 403', async () => {
    const res = await request(app)
      .post('/api/products')
      .set('Authorization', `Bearer ${token}`)
      .send({ name: 'Forbidden', description: 'x', shortDescription: 'x', price: 100 });
    expect(res.status).toBe(403);
  });
});
...
describe('PUT /api/products/:id (admin)', () => {
  test('admin cập nhật sản phẩm → 200', async () => {
    const res = await request(app)
      ...
      .send({
        ...
        shortDescription: 'Ngắn cập nhật',
        price: 6_000_000,
        stockQuantity: 50,
        images: [],
      });
  ...
  test('không auth → 401', async () => {
    const res = await request(app).put(`/api/products/${product.id}`).send({ name: 'NoAuth' });
    expect(res.status).toBe(401);
  });
});

describe('DELETE /api/products/:id (admin)', () => {
  test('admin xóa sản phẩm → 200 hoặc 400 (có ràng buộc)', async () => {
    // Tạo sản phẩm riêng để xóa không ảnh hưởng product chung
    const { Product, ProductVariant } = require('@models');
    const tempProduct = await Product.create({
      nameVi: `__HTTP_ProdComp_Del_${TS}`,
      nameEn: `__HTTP_ProdComp_Del_${TS}`,
      baseName: `__HTTP_ProdComp_Del_${TS}`,
      slug: `http-prod-comp-del-${TS}`,
      basePrice: 1_000_000,
      categoryId: cat.id,
      brandId: brand.id,
      status: 'active',
    ...
    await tempProduct.destroy({ force: true }).catch(() => {});
  });
