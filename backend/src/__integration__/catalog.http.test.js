...
const { User, Category, Brand, Collection, ProductCollection } = require('@models');
...
beforeAll(async () => {
  ...
  collection = await Collection.create({
    nameVi: `__HTTP_Col_${TS}`,
    nameEn: `__HTTP_Col_${TS}`,
    slug: `http-col-${TS}`,
    isActive: true,
  });
  await ProductCollection.create({ productId: prod.id, collectionId: collection.id });
});

afterAll(async () => {
  await ProductCollection.destroy({ where: { collectionId: collection?.id }, force: true }).catch(
    () => {},
  );
  if (collection) await collection.destroy({ force: true }).catch(() => {});
...
describe('GET /api/collections/slug/:slug', () => {
  test('→ 200', async () => {
    const res = await request(app).get(`/api/collections/slug/${collection.slug}`);
    expect(res.status).toBe(200);
  });
});

describe('GET /api/collections/slug/:slug/products', () => {
  test('→ 200', async () => {
    const res = await request(app).get(`/api/collections/slug/${collection.slug}/products`);
    expect(res.status).toBe(200);
  });
});
...
describe('POST /api/collections (admin)', () => {
  test('không auth → 401', async () => {
    const res = await request(app).post('/api/collections').send({ nameVi: 'X' });
    expect(res.status).toBe(401);
  });
});
