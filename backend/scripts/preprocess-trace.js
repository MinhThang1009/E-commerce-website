...
// Bước 3: Security gates
const injection = isPromptInjection(query);
const offTopic = isOffTopic(normalized);
const intent = classifyIntent(normalized);

// Bước 4: Version number extraction (simulate keyword fallback logic)
...
const approxMatch= normalized.toLowerCase().match(new RegExp(`(?:tầm|khoảng|around|budget|about)\\s*${NUM}\\s*${PRICE_UNIT}`, 'i'));
...
const negationTerms = negMatch
  ? negMatch[1].toLowerCase().split(/[\s,/]+/).filter(w => w.length > 2)
  : [];
