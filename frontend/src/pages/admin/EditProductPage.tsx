...
const EditProductPage: React.FC = () => {
  ...
  const {
    ...
    form,
    isEditMode: true, // Thêm prop d? báo là edit mode
    attributes,
    variants,
    onSubmit: async (values: ProductFormData) => {
      ...
      try {
        ...
        // X? lý mô t?: chuy?n ?nh base64 n?u c?n
        ...
        // Xây d?ng d?i tu?ng c?p nh?t d?y d?
        ...
        // Thu?c tính và bi?n th? - luôn g?i n?u có d? an toàn
  ...
  // State d? theo dõi quá trình t?i d? li?u
  const [isDataLoaded, setIsDataLoaded] = useState(false);

  // N?p d? li?u s?n ph?m vào form
  useEffect(() => {
    if (productResponse?.data) {
      // X? lý c? format { product } và raw product
      const product = (productResponse.data as any).product || productResponse.data;

      // X? lý mô t?: x? lý ?nh base64
      let processedDescription = product.description || '';

      // N?u mô t? là chu?i JSON, parse tru?c
      if (
        ...
        try {
          ...
          // N?u parse th?t b?i, gi? nguyên
        }
      }

      // Tru?ng h?p khác: mô t? r?ng nhung có m?ng images, th? t?o t? images
      if (
      ...
      form.setFieldsValue({
        ...
        faqs: product.faqs || [],
        specifications: (() => {
          // T?i thông s? t? b?ng productSpecifications
          if (
      ...
      // C?p nh?t state thông s? k? thu?t
      if (
        product.productSpecifications &&
      ...
      if (product.variants) {
        const formattedVariants: ProductVariant[] = product.variants.map(
          (variant: any, index: number) => ({
            ...
            // S? d?ng stockQuantity thay vì stock d? dúng v?i d? li?u API
      ...
      // Validate form sau khi t?i d? li?u (không thêm validateForm vào dependencies)
      setTimeout(() => {
        // Validate th? công, không dùng hàm validateForm
        const values = form.getFieldsValue();
        const errors = form.getFieldsError();

        // Ki?m tra t?t c? tru?ng b?t bu?c dã di?n chua
        ...
        // Ki?m tra có l?i validation nào không
  ...
  // Hàm h? tr? d?nh d?ng thông báo l?i
  ...
  // X? lý tr?ng thái loading và error
  if (isLoadingProduct) {
    return (
