'use strict';
const { createNewsSchema, feedbackSchema } = require('./content-validator');

describe('createNewsSchema (Zod)', () => {
  it('chấp nhận dữ liệu hợp lệ', () => {
    const r = createNewsSchema.safeParse({
      title: 'Test Title',
      content: 'Nội dung dài hơn 10 ký tự nhé',
    });
...
describe('feedbackSchema (Zod)', () => {
  const valid = {
    ...
    content: 'Nội dung đủ dài hơn 10 ký tự nhé',
  };
  it('chấp nhận dữ liệu hợp lệ', () => expect(feedbackSchema.safeParse(valid).success).toBe(true));
  it('lỗi khi email không hợp lệ', () =>
    expect(feedbackSchema.safeParse({ ...valid, email: 'invalid' }).success).toBe(false));
});
