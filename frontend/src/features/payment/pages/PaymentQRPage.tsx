...
import { useCreateVNPayUrlMutation } from '../api/vnpay-api';
...
const PaymentQRPage: React.FC = () => {
  ...
  const copy = async (text: string, field: string) => {
    try {
      ...
      /* clipboard API không khả dụng — bỏ qua */
    }
  };
  ...
  const formatVND = (n: number) =>
    new Intl.NumberFormat(getLocale(), { style: 'currency', currency: 'VND' }).format(n);
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-2xl max-w-sm w-full p-8 text-center">
            ...
            <h3 className="text-2xl font-bold text-neutral-800 dark:text-neutral-100 mb-2">
              {t('paymentQR.successTitle')}
            </h3>
            <p className="text-neutral-500 dark:text-neutral-400 mb-4">
              {t('paymentQR.successDesc')}
            </p>
            <p className="text-sm text-neutral-400">
              {t('paymentQR.successRedirect', { count: countdown })}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-md p-6">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="px-2 py-0.5 bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400 rounded-full text-xs font-medium">
                    {t('paymentQR.pendingPayment')}
                  </span>
                </div>
            ...
            <div
              className={`rounded-2xl p-4 text-center ${isExpired ? 'bg-red-50 dark:bg-red-900/20' : 'bg-amber-50 dark:bg-amber-900/20'}`}
            >
              <div className="flex items-center justify-center gap-2">
                <span className="text-xl">⏱️</span>
                <span
                  className={`font-semibold text-lg ${isExpired ? 'text-red-600' : 'text-amber-600 dark:text-amber-400'}`}
                >
                  {isExpired ? t('paymentQR.expired') : formatTime(timeLeft)}
                </span>
              </div>
              <p className="text-xs text-neutral-500 mt-1">
                {isExpired ? t('paymentQR.willCancel') : t('paymentQR.timeLeft')}
              </p>
            </div>
                ...
                onClick={async () => {
                  if (orderId) {
                    try {
                      ...
                      addNotification({
                        type: 'success',
                        message: t('paymentQR.cancelSuccess'),
                        duration: 3000,
                      });
                    } catch {
                      /* lỗi huỷ đơn — tiếp tục navigate */
                    }
                    navigate(ROUTES.ORDERS);
                  }
                }}
          ...
          {import.meta.env.DEV && (
            <div className="lg:col-span-3 space-y-4">
              <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-md p-6">
                ...
                {card && (
                  <div
                    className={`bg-gradient-to-br ${card.color} rounded-2xl p-6 text-white mb-5 shadow-lg relative overflow-hidden`}
                  >
                      ...
                      <div className="flex items-start justify-between mb-8">
                        <div>
                          ...
                          <p className="font-bold text-lg">
                            {card.logo} {card.bank.split('–')[0].trim()}
                          </p>
                        </div>
                        <div className="text-right">
                          <span
                            className={`text-xs px-2 py-1 rounded-full font-medium ${card.type === 'success' ? 'bg-green-400/30 text-green-200' : 'bg-red-400/30 text-red-200'}`}
                          >
                            {card.note}
                          </span>
                        </div>
                ...
                <div className="space-y-2 mb-5">
                  {cardFields.map(({ label, value, field }) => (
                    <div
                      key={field}
                      className="flex items-center justify-between bg-neutral-50 dark:bg-neutral-700/50 rounded-xl px-4 py-3"
                    >
                      <span className="text-sm text-neutral-500 dark:text-neutral-400 w-28 flex-shrink-0">
                        {label}
                      </span>
                      <span className="font-mono font-medium text-neutral-800 dark:text-neutral-200 flex-1 text-center">
                        {value}
                      </span>
                      <button
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
                  ...
                  <ol className="text-xs text-blue-700 dark:text-blue-400 space-y-1 list-decimal list-inside">
                    ...
                    <li>
                      <strong>{t('paymentQR.guide4')}</strong>
                    </li>
              ...
              <div className="flex items-center justify-center gap-6 text-xs text-neutral-400 dark:text-neutral-500">
                <span className="flex items-center gap-1">
                  <span>🔐</span> {t('paymentQR.sslEncrypted')}
                </span>
                <span className="flex items-center gap-1">
                  <span>🛡️</span> {t('paymentQR.pciDss')}
                </span>
                <span className="flex items-center gap-1">
                  <span>✅</span> {t('paymentQR.vnpayCertified')}
                </span>
              </div>
            </div>
          )}
