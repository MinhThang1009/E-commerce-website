...
beforeEach(() => {
  ...
  // Mặc định mock axios.get (MyMemory fallback) → fail để không ảnh hưởng test
...
describe('translateBatch — early-return guards (no API call)', () => {
  ...
  it('không gọi axios.post khi OPENROUTER_API_KEY chưa được đặt (dùng MyMemory fallback)', async () => {
    delete process.env.OPENROUTER_API_KEY;
    // MyMemory fallback dùng axios.get — mock để trả về fallback
    axios.get = jest.fn().mockRejectedValue(new Error('network'));
    const texts = ['xin chào'];
    const result = await translateBatch(texts);
    expect(axios.post).not.toHaveBeenCalled();
    expect(result).toEqual(texts); // MyMemory fail → trả nguyên texts
  });

  it("không gọi axios.post khi OPENROUTER_API_KEY === 'demo-key'", async () => {
...
describe('translateBatch — content null/undefined branch', () => {
  beforeEach(() => {
    // MyMemory fallback fail → trả nguyên texts
    axios.get = jest.fn().mockRejectedValue(new Error('network'));
  });

  it('trả về nguyên texts khi content là null (OpenRouter trả null, MyMemory fail)', async () => {
  ...
  it('trả về nguyên texts khi choices mảng rỗng (MyMemory fail)', async () => {
...
describe('translateBatch — catch branch khi JSON.parse thất bại', () => {
  it('trả về texts nguyên khi content không phải JSON hợp lệ (silent fallback)', async () => {
    ...
    expect(result).toEqual(texts);
    // New behavior: silent return without logger.warn (MyMemory also fails via beforeEach mock)
  });
});
