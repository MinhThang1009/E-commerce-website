...
describe('getAllCategories — không có cacheStore', () => {
  ...
  it('có cacheStore nhưng category không có trong countMap → productCount = 0', async () => {
...
describe('getAllBrands — category slug không tồn tại', () => {
  it('category slug không tìm thấy → catId = -1, vẫn gọi findBrandIdsByCategoryId(-1)', async () => {
    ...
    expect(catalogRepository.findAllBrands).toHaveBeenCalledWith({ filter: { idIn: [] } });
  });
});

// ════════════════════════════════════════════════════════════════════════════
// getAllCollections — line 233 (isActive !== undefined)
// Nhánh: isActive = undefined → filter rỗng (không set isActive)
// ════════════════════════════════════════════════════════════════════════════

describe('getAllCollections — isActive undefined', () => {
  it('isActive = undefined → filter rỗng, không set isActive', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.findAllCollections.mockResolvedValue([]);

    await service.getAllCollections({ isActive: undefined });

    expect(catalogRepository.findAllCollections).toHaveBeenCalledWith({ filter: {} });
  });

  it('isActive = false (string) → filter.isActive = false', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.findAllCollections.mockResolvedValue([]);
...
// ════════════════════════════════════════════════════════════════════════════
// getAllProducts — line 419: collection là array của slugs
// Nhánh: cIds.length = 0 nhưng cSlugs.length > 0 → filter.collectionSlugsIn set
// ════════════════════════════════════════════════════════════════════════════

describe('getAllProducts — collection slug array', () => {
  it('collection là array chỉ gồm slugs → collectionSlugsIn được set', async () => {
    const { service, catalogRepository } = makeService();
  ...
  it('collection là array chỉ gồm numeric ids → collectionIdsIn được set', async () => {
    const { service, catalogRepository } = makeService();
    catalogRepository.findProductsList.mockResolvedValue({ count: 0, rows: [] });
    ...
    expect(catalogRepository.findProductsList).toHaveBeenCalledWith(
      expect.objectContaining({
        filter: expect.objectContaining({ collectionIdsIn: expect.arrayContaining(['1', '2']) }),
      }),
    );
  });
