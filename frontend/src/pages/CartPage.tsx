...
const CartPage: React.FC = () => {
  ...
  // API hooks - ch? g?i khi dã xác th?c
  const {
    data: serverCart,
    error: cartError,
  ...
  // Ki?m tra gi? hàng (ki?m tra t?n kho/thay d?i giá) - ch? khi dã xác th?c
  ...
  // X? lý khi MoMo redirect v? thành công
  useEffect(() => {
    ...
    if (status === 'momo-return' && resultCode === '0') {
      ...
      // Cung th? xóa gi? hàng trên server tr?c ti?p d? ch?c ch?n 100%
      if (isAuthenticated) {
        clearServerCart();
      }
  ...
  // Kh?i t?o gi? hàng khi component mount
  useEffect(() => {
  ...
  useEffect(() => {
    if (cartError) {
      console.error('L?i gi? hàng:', cartError);
  ...
  // Tính l?i n?u t?ng don hàng thay d?i và voucher dang du?c áp d?ng
  const handleVoucherRevalidate = async () => {
    ...
    try {
      ...
      // Voucher không còn h?p l? v?i giá tr? gi? hàng hi?n t?i
  ...
  // X? lý thanh toán - truy?n d? li?u voucher vào state
